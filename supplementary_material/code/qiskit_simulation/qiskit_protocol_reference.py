...
def build_composed_staged_emulation_candidate(input_state='0'):
    ...
    qc.metadata = {'candidate_type': 'faithful_v7_three_explicit_bell_resources', 'input_state': input_state, 'valid_expected_outputs': valid_outputs, 'measurement_order': 'out[2] out[1] out[0]', 'success_definition': 'sum of counts over valid_expected_outputs', 'paper_resource_qubits': 9, 'swap_ancillas': 'reuse of consumed/dead measured-stage wires', 'not_implemented': ['QKD', 'authentication', 'network separation', 'abstract arbitration']}
    return qc
...
def build_optimized_staged_candidate(input_state='0'):
    ...
    qc.measure(0, 2)
    valid_outputs = sorted(composed_candidate_expected_outputs(input_state))
    qc.metadata = {'candidate_type': 'optimized_staged_reset_reuse', 'input_state': input_state, 'valid_expected_outputs': valid_outputs, 'measurement_order': 'out[2] out[1] out[0]', 'success_definition': 'sum of counts over valid_expected_outputs', 'not_paper_resource_count': True}
    return qc
import csv
